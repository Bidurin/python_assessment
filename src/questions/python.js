...
// 75 questions | 15 sections | Intermediate–Hard
  ...
  // ── 13. Classes & OOP ★ ──────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      },
      {
        ...
        options: [
          'Dog(Rex) and Dog(Rex)',
          'Dog(Rex) and Dog(name=\'Rex\')',
          'Dog(name=\'Rex\') and Dog(Rex)',
          'Error'
        ],
        answer: 1,
        explain: '__str__ is used by print/str() → "Dog(Rex)". __repr__ is used by repr() → "Dog(name=\'Rex\')". They serve different purposes.'
      },
      {
        q: 'What is the output?\n\nclass BankAccount:\n    def __init__(self, balance):\n        self.__balance = balance\n    def deposit(self, amount):\n        self.__balance += amount\n    def get_balance(self):\n        return self.__balance\n\nacc = BankAccount(100)\nacc.deposit(50)\nprint(acc.get_balance())\nprint(acc.__balance)',
        options: [
          '150 and 150',
          '150 then AttributeError',
          '100 then AttributeError',
          'Error on deposit'
        ],
        answer: 1,
        explain: '__balance is name-mangled to _BankAccount__balance. get_balance() returns 150. Direct acc.__balance raises AttributeError — it is private.'
      },
      {
        q: 'What does this print?\n\nclass Shape:\n    def area(self):\n        return 0\n\nclass Square(Shape):\n    def __init__(self, s):\n        self.s = s\n    def area(self):\n        return self.s ** 2\n\nshapes = [Shape(), Square(3), Square(4)]\nprint(sum(s.area() for s in shapes))',
        options: ['25', '0', 'Error — Shape has no side', '9'],
        answer: 0,
        explain: 'Polymorphism: each object uses its own area(). Shape.area()=0, Square(3).area()=9, Square(4).area()=16. Sum=25.'
      }
    ]
  },
